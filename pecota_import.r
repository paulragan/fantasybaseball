...
hitters <- read.csv("HitterProjections_2018.csv", header = TRUE)
pitchers <- read.csv("PitcherProjections_2018.csv", header = TRUE)
...
# hitters$EligiblePositionss <- ifelse(trimws(hitters$EligiblePositions) == "C", "C, UTIL",
#                              ifelse(hitters$EligiblePositions == "1B", "1B, IF, UTIL",
#                              ifelse(hitters$EligiblePositions == "2B", "2B, IF, UTIL",
#                              ifelse(hitters$EligiblePositions == "3B", "3B, IF, UTIL",
#                              ifelse(hitters$EligiblePositions == "SS", "SS, IF, UTIL",
#                              ifelse(hitters$EligiblePositions == "CF", "CF, OF, UTIL",
#                              ifelse(hitters$EligiblePositions %in% c("RF","LF","OF"), "OF, UTIL",
#                              hitters$EligiblePositions)))))))

hitters$EligiblePositions <- ifelse(trimws(hitters$EligiblePositions) == "C", "C",
                                    ifelse(hitters$EligiblePositions == "1B", "1B",
                                           ifelse(hitters$EligiblePositions == "2B", "2B",
                                                  ifelse(hitters$EligiblePositions == "3B", "3B",
                                                         ifelse(hitters$EligiblePositions == "SS", "SS",
                                                                ifelse(hitters$EligiblePositions == "CF", "OF",
                                                                       ifelse(hitters$EligiblePositions %in% c("RF","LF","OF"), "OF",
                                                                              hitters$EligiblePositions)))))))


hitters$eligibility_C <- ifelse(trimws(hitters$EligiblePositions) == "C", 1,0)   
hitters$eligibility_1B <- ifelse(trimws(hitters$EligiblePositions) == "1B", 1,0)   
hitters$eligibility_2B <- ifelse(trimws(hitters$EligiblePositions) == "2B", 1,0)   
hitters$eligibility_3B <- ifelse(trimws(hitters$EligiblePositions) == "3B", 1,0)
hitters$eligibility_SS <- ifelse(trimws(hitters$EligiblePositions) == "SS", 1,0)
hitters$eligibility_IF <- ifelse(trimws(hitters$EligiblePositions) %in% c("1B", "2B", "SS", "3B"), 1, 0)
hitters$eligibility_CF <- ifelse(trimws(hitters$EligiblePositions) == "CF", 1,0)
hitters$eligibility_OF <- ifelse(trimws(hitters$EligiblePositions) %in% c("RF","LF","OF","CF"), 1,0)
hitters$eligibility_UTIL <- 1

# names
hitters <- as.data.frame(append(hitters, list(lastnamefirstname = paste(hitters$LASTNAME, hitters$FIRSTNAME, sep=", ")), after=3))
hitters <- as.data.frame(append(hitters, list(name = paste(hitters$FIRSTNAME, hitters$LASTNAME, sep=" ")), after=3))
pitchers <- as.data.frame(append(pitchers, list(lastnamefirstname = paste(pitchers$LASTNAME, pitchers$FIRSTNAME, sep=", ")), after=3))
pitchers <- as.data.frame(append(pitchers, list(name = paste(pitchers$FIRSTNAME, pitchers$LASTNAME, sep=" ")), after=3))

# net stolen bases
hitters <- as.data.frame(append(hitters, list(net_sb = (hitters$SB - hitters$CS)), after=25))
...
# positional_rank #
# ############### #
hitters_min <- transform(hitters_min, positional_rank = ave(hitters_min$combined_rank, hitters_min$EligiblePositions, FUN = function(x) rank(x, ties.method = "first")))
...
hitters_min <- transform(hitters_min, positional_stdev_rank = ave(hitters_min$stdev_rank, hitters_min$EligiblePositions, FUN = function(x) rank(x, ties.method = "first")))

# ORDER
hitters_min <- hitters_min[order(hitters_min$EligiblePositions,hitters_min$EligiblePositionsitional_stdev_rank),]

# Run Standard deviations again using averages among positions
hitters_min$likely_drafted <- 0
hitters_min$likely_drafted <- ifelse((hitters_min$EligiblePositionsitional_stdev_rank <= 65 & hitters_min$EligiblePositions == "OF") |
                                       (hitters_min$EligiblePositionsitional_stdev_rank <= 20 & hitters_min$EligiblePositions %in% c("1B", "2B", "3B", "SS")) |
                                       (hitters_min$EligiblePositionsitional_stdev_rank <= 14 & hitters_min$EligiblePositions == "C"), 1,0)
...
# Re-Run standard deviations among likely drafted
hitters_min$drafted_rbi_sdev <- 


write.csv(hitters_min, file="hitters.csv", row.names = FALSE)
